...
conjecture  I ~= J -> (state(cache_var(I)) = e -> state(cache_var(J)) = i) & (state(cache_var(I)) = s -> state(cache_var(J)) = i | state(cache_var(J)) = s)
...
conjecture (cmd(chan3(I)) = invack & curCmd ~= empty & exGntd = true) ->  ((J ~= I) -> (state(cache_var(J)) ~= e & cmd(chan2(J)) ~= gnte & cmd(chan3(J)) ~= invack))
